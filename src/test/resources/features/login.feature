Feature: Login Form Validation
  ...
  Scenario Outline: UC-1 Test Login Form with Empty Credentials
    ...
    Examples:
      ...
      | standard_user1   | secret_sauce | Epic sadface: Username is required |
      | locked_out_user1 | secret_sauce | Epic sadface: Username is required |
  ...
  Scenario Outline: UC-2 Test Login Form with credentials by passing Username
    ...
    Examples:
      | Username    | Password     | ErrorMessage                       |
      | error_user  | secret_sauce | Epic sadface: Password is required |
      | visual_user | s            | Epic sadface: Password is required |
